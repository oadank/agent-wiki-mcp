...
async function main() {
  if (HTTP_PORT) {
    // HTTP 模式：stateless，每个请求新建 McpServer 实例（工具在 createWikiServer 中注册）
    const httpServer = createServer(async (req, res) => {
      ...
      // 每个请求新建 McpServer 实例 + transport（stateless 模式）
      const requestServer = createWikiServer();
      ...
      // 处理请求
      // 补全 Accept header（兼容不发送 text/event-stream 的客户端）
      // rawHeaders 是只读数组，用 Object.defineProperty 重写 getter
      const origRawHeaders = req.rawHeaders;
      const hasTextES = origRawHeaders.some((h, i) => i % 2 === 0 && h.toLowerCase() === 'accept') 
        && origRawHeaders.some((h, i) => i % 2 === 1 && h.toLowerCase().includes('text/event-stream'));
      const hasAppJSON = origRawHeaders.some((h, i) => i % 2 === 0 && h.toLowerCase() === 'accept') 
        && origRawHeaders.some((h, i) => i % 2 === 1 && h.toLowerCase().includes('application/json'));
      if (!hasTextES || !hasAppJSON) {
