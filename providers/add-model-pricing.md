...
#### ℹ️ Info
This is different from [[.completion/model_alias|`model_alias_map`]], which is a runtime SDK/proxy feature for mapping user-facing model names to LiteLLM model identifiers. The `aliases` field here is for the model cost JSON only — it avoids duplicate entries for models that share identical pricing and capabilities.
