...
OpenHuman 基于 GNU GPL3 开源。本页是系统的高层轮廓；深度开发者架构参考位于仓库中的 [[.architecture.zh-CN|深度架构文档]]。
...
1. **连接**。通过 OAuth 接入[[..features/integrations/README.zh-CN|集成]]。后端保存 token；核心永远不会以明文形式看到它。
2. **自动获取**。每二十分钟，[[..features/obsidian-wiki/auto-fetch.zh-CN|调度器]]会遍历每个活跃连接，并要求每个原生提供商进行同步。
...
9. **压缩**。工具输出和大型源数据在进入 LLM 上下文前经过 [[..features/token-compression.zh-CN|TokenJuice]] 处理。
