...
完整集合来自为[[README.zh-CN|第三方集成]]提供支持的 [Composio](https://composio.dev) 连接器层。当连接活跃时，相关的触发器订阅会自动接入。
...
- **`escalate`** 是重型路径。当分类智能体决定触发器需要真正的工作时，它将自包含的任务描述交给 Orchestrator。orchestrator 可以访问你完整的技能表面、工具、记忆和[[.subconscious.zh-CN|潜意识循环]]输出。从那里它可能：
  - 起草一封重要邮件的回复并排队等待你批准。
  ...
  - 判断触发器意味着一个会议刚刚被预定并为该通话预加载[[.mascot/meeting-agents.zh-CN|会议智能体]]。
...
分类在快速模型层运行（参见[[.model-routing/README.zh-CN|自动模型路由]]），所以分类本身在亚秒级完成。
...
触发器遵循与产品其余部分相同的边界（参见[[.privacy-and-security.zh-CN|隐私与安全]]）：
...
* [[README.zh-CN|第三方集成]]，触发器来源的服务目录。
* [[.obsidian-wiki/auto-fetch.zh-CN|从集成自动拉取]]，轮询对应部分，定期将源数据摄入记忆树。
* [[.subconscious.zh-CN|潜意识循环]]，使用触发器上下文和记忆提前规划的背景循环。
* [[.mascot/meeting-agents.zh-CN|会议智能体]]，升级触发器可以落地的地方之一（日历事件有 Meet 链接）。