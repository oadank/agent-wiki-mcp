...
会议的入站音频被捕获并实时推送通过流式语音转文字。转录按说话者分离，经过与桌面听写相同的幻觉过滤和后处理，并在会议展开时折叠到[[.obsidian-wiki/memory-tree.zh-CN|记忆树]]中——在正确的人、正确的主题、正确的项目下，带有吉祥物以后可以使用的反向链接。
...
对话轮次通过快速模型层路由（参见[[.model-routing/README.zh-CN|自动模型路由]]），这样延迟感觉像在和一个正在倾听的人说话，而不是等待聊天机器人。
...
- [[.obsidian-wiki/memory-tree.zh-CN|**记忆树**]]——召回之前的会议、决策、开放线程、谁上次说了什么、承诺了什么。
- [[.obsidian-wiki/auto-fetch.zh-CN|**从集成自动拉取**]]和[[.integrations/README.zh-CN|**第三方集成**]]——从 Slack 拉取线程、一封邮件、一个 Linear ticket、一个 Notion 文档、一个日历条目、一个 Drive 文件。
- [[.native-tools/README.zh-CN|**原生工具**]]——搜索网络、抓取页面、运行快速代码/数据查询，全部不离开通话。
- [[.subconscious.zh-CN|**潜意识循环**]]输出——任何它在后台一直在工作的东西都随手可得。
...
- **转录和记忆。** 实时转录以与任何其他来源相同的方式落在[[.obsidian-wiki/memory-tree.zh-CN|记忆树]]中——在通话中的人、项目和出现的主题下。它们是本地优先的，遵循项目的[[.privacy-and-security.zh-CN|隐私与安全]]规则。
...
- 语音管道 - `src/openhuman/voice/`（STT in、TTS out、幻觉过滤、后处理）。参见[[.native-tools/voice.zh-CN|原生语音]]。
- 作为出站摄像头的吉祥物画布 - `app/src/features/meet/MascotFrameProducer.tsx` 和 Tauri 端 `mascot_native_window.rs` 窗口。
- 嵌入式 Meet webview - 参见 [[..developing/cef.zh-CN|Chromium Embedded Framework]]。与六个零注入提供商不同，Google Meet 使用 CDP 驱动 join 自动化：CDP 通过 `Page.addScriptToEvaluateOnNewDocument` 和 `Runtime.evaluate` 注入 bridging scripts / recipes（`GOOGLE_MEET_RECIPE_JS` / `provider_recipe_js("google-meet")`），而不是真正的零注入交付。
- 要阅读的上下文的重要 commit - `0bc74575`（实时记笔记）、`f1203479`（真实 LLM 轮次 + 调优 TTS）、`b6d05cb4`（吉祥物画布作为出站摄像头）、`f5dce783`（吉祥物帧流水线 + 屏外会议窗口）。

## 另见

- [吉祥物](./)——屏幕上的角色本身，会议之外。
- [[.native-tools/voice.zh-CN|原生语音]]——会议智能体所依赖的 STT / TTS。
- [[.obsidian-wiki/memory-tree.zh-CN|记忆树]]——转录和决策落地的地方。
- [[.native-tools/README.zh-CN|原生工具]]——吉祥物在通话中可以伸手拿什么。
- [[.model-routing/README.zh-CN|自动模型路由]]——对话轮次为什么感觉低延迟。