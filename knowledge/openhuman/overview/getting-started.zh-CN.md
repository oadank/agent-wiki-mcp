...
OpenHuman 支持 **macOS、Windows 和 Linux** 桌面端。建议 4 GB 以上内存；如果要摄入超大型邮箱或仓库，或在同一台机器上运行[[.features/model-routing/local-ai.zh-CN|本地模型]]，建议 16 GB 以上。

### 权限

首次启动 OpenHuman 时，操作系统会提示授予应用所需的权限（macOS 上的 Accessibility、语音热键的 Input Monitoring，以及计划使用[[.features/mascot/meeting-agents.zh-CN|会议智能体]]时的相机/麦克风）。你随时可以在 **设置 → 自动化与渠道** 中查看和调整这些权限。
...
* [[.features/mascot/meeting-agents.zh-CN|**会议智能体**]] —— 放入一个 Google Meet 链接，吉祥物作为真实参与者加入：它倾听、将笔记记入记忆树、在通话中说话，并实时使用工具。
* [[.features/obsidian-wiki/auto-fetch.zh-CN|**从集成自动拉取**]] —— 从**设置**中连接更多源；每二十分钟调度器将新数据拉入你的树。
* [[.features/native-tools/voice.zh-CN|**原生语音**]] —— 按键说话输入和 TTS 回复，这样你可以和 OpenHuman 对话而不是打字。
* [[.features/subconscious.zh-CN|**潜意识循环**]] —— 让你离开时吉祥物继续处理待办任务。
